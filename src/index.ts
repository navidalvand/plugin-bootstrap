...
const postGeneratedHandler = async ({
  ...
  logger.debug('[POST] Calling composeState with CUSTOM_TRENDS');
  ...
  // 1️⃣  first composeState – where you pick providers
  let state = await runtime.composeState(
    message,
    undefined,
    ['PROVIDERS', 'CHARACTER', 'ENTITIES', 'CUSTOM_TRENDS'] //  <-- add this
  );

  logger.debug(
    '[POST] providerData keys:',
  ...
  const prompt = composePromptFromState({
    state,
    template: providers.customPostCreationTemplate,
  });

  logger.trace('[POST] Prompt sent to TEXT_SMALL model:\n', prompt);
...
export const bootstrapPlugin: Plugin = {
  ...
  providers: [
    ...
    providers.customTrendsProvider,
  ],
